...
--
-- What this does:
--   Replaces the text logo line in body_html
--     <p ...>Culligan Group · Purezza Premium Water</p>
--   with a horizontal row of THREE brand logos served from the public
--   signature-assets bucket (Culligan, then Purezza, then Zip — Jordan's locked
--   order). Both brand templates (purezza + culligan_zip) carry all three logos.
--
-- Logos are used as-sent (solid backgrounds, no transparency stripping) per
-- Jordan's locked decision. Explicit width/height attributes preserve aspect
-- ratios at 40px tall: Culligan 1280x720→71x40, Purezza 160x160→40x40,
-- Zip 175x74→95x40.
--
-- body_text is left untouched — it already carries the text fallback
-- "Culligan Group · Purezza Premium Water" from the density migration, which is
-- the correct plaintext rendering when an email client can't load images.
--
-- Everything else in body_html (name/title/brand block, {{sending_mailbox_email}}
-- line, licence line, BOTH disclaimer paragraphs, density styling: name block
-- line-height 1.3, disclaimers 9px/1.3) is preserved verbatim from the density
-- migration.
--
-- Scope: Jordan's user_id + org_id, per brand_key — never touches another
-- user's row even if copy coincides.
--
-- Idempotent: the WHERE clause skips rows whose body_html already references the
-- Culligan logo asset, so a re-run is a no-op.
...
BEGIN
  ...
  -- Purezza row (brand line: "Purezza Australia").
  ...
  -- Culligan Zip row (brand line: "Culligan Group").
